...
namespace SharedAssembly.Tests.UnitTests
{
    public class TwoAncientPoems5CommandHandlerTests
    {
        ...
        public async Task Should_handle_TwoAncientPoems_5_Command()
        {
            ...
            // Assert
            (await repository.Query("TwoAncientPoems_5", "abc")).Should().HaveCount(1);
        }
        [Fact]
        public async Task Should_persist_another_BabyWhiteCloudCommand()
        {
            ...
            // Assert
            (await repository.Query("TwoAncientPoems_5")).Should().HaveCountGreaterOrEqualTo(2);
        }
        [Fact]
        public async Task Should_update_the_same_BabyWhiteCloudCommand()
        {
            ...
            // Assert
            (await repository.Query("TwoAncientPoems_5", "abc")).Should().HaveCount(1);
            (await repository.Query("TwoAncientPoems_5", "abc")).Single().Answers.Should().IsSameOrEqualTo(new[] {"e", "b", "c", "d", "a"});
        }
        [Fact]
        ...
        public async Task Should_complete_test_when_answers_are_correct()
        {
            ...
            // Assert
            (await repository.Query("TwoAncientPoems_5", "abc")).Single().CompleteAt.Should().NotBeNull();
            ...
            // Assert
            (await repository.Query("TwoAncientPoems_5", "abc")).Single().CompleteAt.Should().BeNull();
        }
        [Fact]
